...
from receler.estimate_memory import estimate_training_memory, check_gpu_memory, recommend_config
...
def train_step_prompt_only(
    ...
    # 提取掩码
    # head_num 设为 None，让 get_mask 自动从 attention maps 推断
    masks = get_mask(
        attn_maps=attn_maps,
...
def train_unlearning_prompt_only(
    ...
    if args.device == "cuda" and torch.cuda.is_available():
        ...
        print("显存检查")
        print("=" * 60)
        total_memory, free_memory = check_gpu_memory(0)
        
        # 估算训练显存需求
        dtype_for_estimation = torch.float16 if args.use_fp16 else torch.float32
        try:
            print(f"\n估算训练显存需求...")
            memory_breakdown = estimate_training_memory(
                model_path=args.model_path,
                batch_size=args.batch_size,
                num_frames=getattr(args, 'num_frames', 17),
                latent_height=getattr(args, 'latent_height', 32),
                latent_width=getattr(args, 'latent_width', 32),
                latent_channels=getattr(args, 'latent_channels', 16),
                dtype=dtype_for_estimation,
                include_vae=False
            )
            
            estimated_total = memory_breakdown.get('total', 0)
            print(f"\n显存对比:")
            print(f"  估算需求: {estimated_total:.2f} GB")
            print(f"  可用显存: {free_memory:.2f} GB")
            
            if estimated_total > free_memory * 0.9:
                print(f"\n⚠️  警告：估算显存需求 ({estimated_total:.2f} GB) 接近或超过可用显存 ({free_memory:.2f} GB)")
                print(f"   建议:")
                print(f"   1. 添加 --use_cpu_offload 参数以使用 CPU offload")
                print(f"   2. 减小 --batch_size（当前: {args.batch_size}）")
                print(f"   3. 使用 --use_fp16 以节省显存（如果尚未启用）")
                
                # 如果未指定 use_cpu_offload，自动推荐
